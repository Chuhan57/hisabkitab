...
def subcatagery_add(request, foo): 
    if request.method == 'POST':
        ...
        cats = Catagery.objects.get(title=foo)
        subs = Subcatagery.objects.create(catagery=cats ,name=addname ,amount=addamount)
        subs.save() 
        return redirect('/')
...
def home(request):   
    ...
    total = Subcatagery.objects.all().aggregate(Sum('amount'))

    data = Catagery.objects.all()
    if request.method == 'POST':
        title =  request.POST['title']
        
        query = Catagery.objects.create(title=title)
        query.save()
